...
class Agent:
    def __init__(self, discretize_levels, gamma, learning_rate, total_exploring_episode):
        ...
        self.discretize_state_levels = 4
        self.discretize_action_levels = 10
        ...
        self.Q = defaultdict(lambda: np.zeros((self.discretize_action_levels,
                                               self.discretize_action_levels,
                                               self.discretize_action_levels,
                                               self.discretize_action_levels)))
    ...
    def discretize_state(self, state):
        ...
        for i in range(len(state)):
            ...
            # min-max normalization
                index = int(50*(state[i] - self.state_space[i][0]) / (self.state_space[i][1] - self.state_space[i][0]))
            else:
                index = int(self.discretize_state_levels*(state[i] - self.state_space[i][0]) / (self.state_space[i][1] - self.state_space[i][0]))

            discrete_states.append(index)
        return tuple(discrete_states)

    # def discretize_action(self, state):
    #     discrete_states = []
    #     for i in range(len(state)):
    #         # min-max normalization
    #         index = int(self.discretize_action_levels*(state[i] - self.state_space[i][0]) / (self.state_space[i][1] - self.state_space[i][0]))
    #         discrete_states.append(index)
    #     return tuple(discrete_states)
    ...
    def epsilon_policy(self, epsilon, state):
        if random.random() < epsilon:
            action = ()
            for i in range(0, 4):
                action += (random.randint(0, self.discretize_action_levels - 1),)
        else:
    ...
    def continuous_action(self, next_action):
        return tuple([(a / self.discretize_action_levels) * 2 - 1 for a in next_action])
