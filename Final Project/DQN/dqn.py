...
if __name__ == "__main__":
    ...
    load_dotenv(dotenv_path="./results_3/.env")
    ...
    main_net = QNet(n_states=n_states, n_actions=n_actions, seed=0, model_type=model_type,
                    discretize_level=discretize_level).to(device)
    target_net = QNet(n_states=n_states, n_actions=n_actions, seed=0, model_type=model_type,
                      discretize_level=discretize_level).to(device)
    ...
    # writer.add_graph(main_net, states)
    start_episode = 0
    ...
    for ep in range(start_episode + 1, total_episodes + 1):
        ...
        for step in range(episode_len):
            ...
            # writer.add_scalar("Loss/train", loss, (ep-1)*total_episodes + step)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
